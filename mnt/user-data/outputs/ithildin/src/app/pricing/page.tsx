  ...
  return (
    <main style={{ background: 'var(--bg)' }}>
      ...
      <section style={{ padding: '160px 52px 60px', textAlign: 'center', maxWidth: 540, margin: '0 auto' }}>
        ...
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 300, color: '#383838', marginBottom: 28 }}>
        ...
        <div style={{ display: 'inline-flex', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 100, padding: 3, gap: 3 }}>
          {(['monthly','annual'] as const).map(b => (
            <button key={b} onClick={() => setBilling(b)} style={{
              ...
              color: billing === b ? 'var(--white)' : '#383838',
        ...
        <div style={{
          ...
          {PLANS.map((plan, i) => (
            <div key={plan.name} className="plan reveal" style={{
              ...
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', fontWeight: 300, color: '#383838', marginBottom: 24 }}>{plan.desc}</p>
              ...
              <div style={{ marginBottom: 24, display: 'flex', alignItems: 'baseline', gap: 4 }}>
                {plan.price ? (
                  <>
                    ...
                    <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', fontWeight: 300, color: '#383838' }}>/mo</span>
                  </>
                ) : (
              ...
              {plan.features.map((f, j) => (
                <div key={j} style={{ display: 'flex', gap: 10, marginBottom: 11 }}>
                  <span style={{ color: '#1e1e1e', fontSize: '0.7rem', marginTop: 2, flexShrink: 0 }}>—</span>
                  <p style={{
                    fontFamily: 'var(--font-sans)', fontSize: '0.78rem', fontWeight: 300,
                    color: j === 0 && i > 0 ? '#222' : '#2e2e2e',
        ...
        {FAQ.map((item, i) => (
          <div key={i} className="reveal" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', transitionDelay: `${i * 0.05}s` }}>
            <button onClick={() => setOpen(open === i ? null : i)} style={{
              ...
              <span style={{ color: '#2e2e2e', fontSize: '1rem', flexShrink: 0, transition: 'transform 0.3s', transform: open === i ? 'rotate(45deg)' : 'none' }}>+</span>
            </button>
            {open === i && (
              <p style={{
                fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 300,
                color: '#383838', lineHeight: 1.7, paddingBottom: 22,
