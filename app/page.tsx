...
function VisUpload({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      y += 2; setScanY(y)
      if (y >= 60) { clearInterval(iv); setTimeout(() => setAiVisible(true), 200) }
    }, 16)
...
function VisBrief({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    if (!active) return
    const timers = BRIEF.map((_, i) => setTimeout(() => setShown(i + 1), i * 380))
    return () => timers.forEach(clearTimeout)
  }, [active])
...
function VisCredibility({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    if (!active) return
    let c = 0
    const iv = setInterval(() => { c++; setCount(c); if (c >= 48) clearInterval(iv) }, 18)
    const rowTimers = CRED_ROWS.map((_, i) => setTimeout(() => setRows(i + 1), 300 + i * 280))
    const barTimer = setTimeout(() => setBars(true), 500)
    return () => { clearInterval(iv); rowTimers.forEach(clearTimeout); clearTimeout(barTimer) }
  }, [active])
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 3, maxWidth: 640, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <p style={{
            ...
            Upload your documents. Depose with live contradiction detection. Review testimony automatically.
          </p>
          <EmailCapture />
          <p style={{
